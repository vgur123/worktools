do $$
    ...
    select OPERATION_DATE, EXT_ID, PART into last_rec
         from tmp_table
         where tmp_table.ID = id_max;
    insert into sbp_transfer (partition, suit, custom_product,      epk_id,                             transfer_version, create_date,    status,       transfer_sum, transfer_currency, card_num,         receiver_phone_number,  doc_id, nspk_id, receiver_first_name,  receiver_last_name,  receiver_middle_name)
            select                    last_rec.PART, SUIT,  upper(CUSTOM_PRODUCT_TYPE), CAST ((RECIPIENT_EPK_ID) AS BIGINT), 1,                OPERATION_DATE, STATUS_VALUE, PAYMENT_SUMMA, CURRENCY,         PAY_TOOL_NUMBER,  RECIPIENT_PHONE_NUMBER, DOC_ID, EXT_ID,  upper(RECIPIENT_FIRST_NAME), upper(RECIPIENT_LAST_NAME), upper(RECIPIENT_MIDDLE_NAME)
            from sbp_b2c_history.history_operation as hst
            where hst.OPERATION_DATE=last_rec.OPERATION_DATE and hst.EXT_ID=last_rec.EXT_ID;
...
--select * from sbp_b2c_history.history_operation where OPERATION_DATE>='2024-01-01' and OPERATION_DATE<='2024-01-30';
